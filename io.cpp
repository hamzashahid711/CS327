...
void io_display(dungeon *d)
{
  ...
  * not otherwise used.                                                 */
  ...
  mvprintw(22, 30, "Nearest visible monster: ");
   mvprintw(23, 30, "PC health %d", d->PC->hp);
...
void spells(dungeon *d){

mvprintw(1, 1, "you have %d mana", d->PC->mana);
mvprintw(2, 1, "Spells:");
mvprintw(3, 2, "a:[heal 100] 3 mana");
mvprintw(4, 2, "b:[spawn a weapon] 3 mana");
mvprintw(5, 2, "c:[teleport] 8 mana");
mvprintw(6, 2, "d:[spawn armor] 3 mana");
mvprintw(10, 1, " hit esc and move key to continue");
int key = 0;
while((key = getch()) != 27){
  if(key == '1'){
  if(d->PC->mana >=3){
  d->PC->mana -=3;
  d->PC->hp +=100;
  mvprintw(7, 2, "you healed up 100", d->PC->mana);
  mvprintw(1, 1, "you have %d mana left", d->PC->mana);
  }else{
    mvprintw(7, 2, "you do not have enough mana");
    mvprintw(9, 2, "pick up potions to earn mana");
...
if(key == '2'){
  ...
  if(d->PC->mana >=3 ){
    ...
    for(int j =0; j < DUNGEON_X;j++){
      if ((d->objmap[i][j]) && d->objmap[i][j]->get_type() ==1 ) {
        if(d->PC->eq[0] == NULL){
          d->PC->eq[0] = d->objmap[i][j];
           d->objmap[i][j]  = NULL;
            break;
        }
     ...
     mvprintw(7, 2, "you dont have enough mana");
  }
  
}else{
  mvprintw(7, 2, "you already have a weapon");
}
}
if(key == '3'){
  if(d->PC->mana >=8){
...
if(key == '4'){
  ...
  if(pass2){
    ...
    for(int i = 0; i< DUNGEON_Y;i++){
    for(int j =0; j < DUNGEON_X;j++){
      if ((d->objmap[i][j]) && d->objmap[i][j]->get_type() ==5 ) {
      
        if(d->PC->eq[4] == NULL){
          d->PC->eq[4] = d->objmap[i][j];
           d->objmap[i][j]  = NULL;
            break;
        }
  ...
  mvprintw(7, 2, "you already have armor");
}
}
}
 refresh();
}
void pickup_book(dungeon *d){

  for(int i = 0; i< DUNGEON_Y;i++){
    for(int j =0; j < DUNGEON_X;j++){
  
    if(d->character_map[i][j] ){
      if(d->character_map[i][j]->symbol == '@'){

              
     if ((d->objmap[i][j]) && d->objmap[i][j]->get_type() == 13) {
       
      
       
       mvprintw(1, 10, "you picked up: %s" , d->objmap[i][j]->get_name());
       d->PC->book = true;
       d->objmap[i][j] = NULL;
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    if(d->PC->book== true){
      ...
      mvprintw(1, 1, "FIND THE SPELL BOOK!");
    }
