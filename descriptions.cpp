...
uint32_t parse_descriptions(dungeon *d)
{
  ...
  file = ("monster_desc.txt");//getenv("HOME");
  // std::cout<<"made it to parse descriptions\n";
  f.open(file.c_str());
  parse_monster_descriptions(f, d, &d->monster_descriptions);
  retval =1;
  f.close();
  file = ("object_desc.txt");
  f.open(file.c_str());
  parse_object_descriptions(f, d, &d->object_descriptions);
  f.close();
  sleep(5);
 // if (file.length() == 0) {
    //file = ".";
